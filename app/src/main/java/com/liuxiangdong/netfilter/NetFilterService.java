...
public class NetFilterService extends VpnService {
    ...
    private ParcelFileDescriptor vpnStart() {
        ...
        for (AppInfo appInfo : AppInfo.getAppInfos(this))
            if (!(wifi ? appInfo.wifi_blocked : appInfo.other_blocked)) {
                try {
                    builder.addDisallowedApplication(appInfo.info.packageName);
    ...
    public static void reload(String network, Context context) {
        if (network == null || ("wifi".equals(network) ? NetworkUtil.isWifiActive(context) : !NetworkUtil.isWifiActive(context))) {
            Intent intent = new Intent(context, NetFilterService.class);
